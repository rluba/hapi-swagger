...
internals.properties.prototype.parseProperty = function (name, joiObj, parent, parameterType, useDefinitions, isAlt) {
  ...
  // add enum
  const describe = internals.describe(joiObj);
...
internals.properties.prototype.parseNumber = function (property, joiObj) {
  ...
  const greater = internals.getArgByName(describe.rules, 'greater');
  if (greater !== undefined) {
      property.minimum = greater;
      property.exclusiveMinimum = true;
  }
  const less = internals.getArgByName(describe.rules, 'less');
  if (less !== undefined) {
      property.maximum = less;
...
internals.describe = function (joiObj) {
    const result = joiObj.describe();
    let base = result.base;
    result.valids = result.valids || [];
    result.flags = result.flags || {};
    while (base) {
        if (base.valids) {
            result.valids = Hoek.merge(result.valids, base.valids);
        }
        if (base.rules) {
            result.rules = Hoek.applyToDefaults(base.rules, result.rules);
        }
        if (base.flags) {
            result.flags = Hoek.applyToDefaults(base.flags, result.flags);
        }
        base = base.base;
    }

    if (!result.valids.length) {
        delete result.valids;
    }
    return result;
};
