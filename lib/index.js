...
const appendDataContext = function (plugin, settings) {
    ...
    plugin.ext('onPostHandler', (request, reply) => {
        ...
        if (response.variety === 'view') {
            ...
            const viewSettings = Hoek.clone(settings);
            if (request.query.tags) {
                // append tags from document request to JSON request
                viewSettings.jsonPath = appendQueryString(settings.jsonPath, 'tags', request.query.tags);
            }
            if (plugin.realm.modifiers.route.prefix) {
                ['jsonPath', 'swaggerUIPath'].forEach((setting) => {
                    viewSettings[setting] = plugin.realm.modifiers.route.prefix + viewSettings[setting];
                });
            }

            response.source.context.hapiSwagger = viewSettings;
        }
